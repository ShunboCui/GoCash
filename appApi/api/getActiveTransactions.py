...
"""
    Returns current ACTIVE transactions done by the user
    Parameters:
        userId: unique user identifier
    Returns:
        userTransactionsList: list of all transactions
"""
class GetActiveTransactionsApi(Resource):
    # mod = transaction
    ...
    def getUserTransactions(self, userId):
        ...
        # transactionsCursor = TransactionInfoCollection.find_one({"$and":[ {"userId":userId}, {"transactionId":transactionId}]})

        transactionsCursor = TransactionInfoCollection.find({"$and":[ {"userId":userId}, {"status":"Initialized"}]})   #mongodb returns a cursor object, which we must iterate through, or else is not serializable 
        pprint(TransactionInfoCollection)
